...
import umlgraph.graphview.arrows.SimpleArrow;
import umlgraph.graphview.arrows.TriangleArrow;
import umlgraph.graphview.edges.Edge;
import umlgraph.graphview.edges.EdgeBase;
import umlgraph.graphview.edges.EdgeCurve;
import umlgraph.graphview.edges.EdgeLine;
import umlgraph.graphview.labels.Label;
import umlgraph.graphview.labels.LabelSource;
import umlgraph.graphview.utils.UtilitiesJavaFX;
import umlgraph.graphview.utils.UtilitiesPoint2D;
import umlgraph.graphview.strategy.PlacementStrategy;
import umlgraph.graphview.strategy.RandomPlacementStrategy;
...
public class GraphPanel<V, E> extends Pane {
    ...
    private Map<umlgraph.graph.Edge<E,V>, Tuple<Vertex<V>>> connections;
    ...
    private void initNodes() {
        ...
        List<umlgraph.graph.Edge<E, V>> edgesToPlace = listOfEdges();

        for (Vertex<V> vertex : vertexNodes.keySet()) {

            Iterable<umlgraph.graph.Edge<E, V>> incidentEdges = theGraph.incidentEdges(vertex);

            for (umlgraph.graph.Edge<E, V> edge : incidentEdges) {
    ...
    private EdgeBase createEdge(umlgraph.graph.Edge<E, V> edge, GraphVertexPaneNode<V> graphVertexInbound, GraphVertexPaneNode<V> graphVertexOutbound) {
        /*
...
//            v.attachLabel(label);
    ...
    private void addEdge(EdgeBase e, umlgraph.graph.Edge<E, V> edge) {
        //edges to the back
    ...
    private void insertNodes() {
        ...
        if (!unplottedVertices.isEmpty()) {
            ...
            for (Vertex<V> vertex : unplottedVertices) {
                ...
                double x, y;
                Collection<umlgraph.graph.Edge<E, V>> incidentEdges = theGraph.incidentEdges(vertex);
                if (incidentEdges.isEmpty()) {
                    /* not (yet) connected, put in the middle of the plot */
                    x = mx;
                    y = my;
                } else {
                    umlgraph.graph.Edge<E, V> firstEdge = incidentEdges.iterator().next();
        ...
        Collection<umlgraph.graph.Edge<E, V>> unplottedEdges = unplottedEdges();
        if (!unplottedEdges.isEmpty()) {
            for (umlgraph.graph.Edge<E, V> edge : unplottedEdges) {
    ...
    private void removeNodes() {
        ...
        Collection<umlgraph.graph.Edge<E, V>> removedEdges = removedEdges();
        for (umlgraph.graph.Edge<E, V> e : removedEdges) {
    ...
    * AUTOMATIC LAYOUT
    ...
    private int getTotalEdgesBetween(Vertex<V> v, Vertex<V> u) {
        ...
        int count = 0;
        for (umlgraph.graph.Edge<E, V> edge : theGraph.edges()) {
    ...
    private List<umlgraph.graph.Edge<E, V>> listOfEdges() {
        List<umlgraph.graph.Edge<E, V>> list = new LinkedList<>();
        for (umlgraph.graph.Edge<E, V> edge : theGraph.edges()) {
            list.add(edge);
        }
        return list;
     ...
     */
    private Collection<umlgraph.graph.Edge<E, V>> removedEdges() {
        List<umlgraph.graph.Edge<E, V>> removed = new LinkedList<>();

        Collection<umlgraph.graph.Edge<E, V>> graphEdges = theGraph.edges();
     ...
     * @return collection of edges
     */
    private Collection<umlgraph.graph.Edge<E, V>> unplottedEdges() {
        List<umlgraph.graph.Edge<E, V>> unplotted = new LinkedList<>();

        for (umlgraph.graph.Edge<E, V> e : theGraph.edges()) {
     ...
     * @return stylable element
     */
    public StylableNode getStylableEdge(umlgraph.graph.Edge<E, V> edge) {
    ...
    public StylableNode getStylableEdge(E edgeElement) {
        for (umlgraph.graph.Edge<E, V> e : edgeNodes.keySet()) {
     ...
     * @return stylable element (label)
     */
    public StylableNode getStylableLabel(umlgraph.graph.Edge<E,V> e) {
