...
import com.rnar.umlgraph.graphview.arrows.DiamondArrow;
import com.rnar.umlgraph.graphview.arrows.SimpleArrow;
import com.rnar.umlgraph.graphview.arrows.TriangleArrow;
import com.rnar.umlgraph.graphview.edges.Edge;
import com.rnar.umlgraph.graphview.edges.EdgeBase;
import com.rnar.umlgraph.graphview.edges.EdgeCurve;
...
import com.rnar.umlgraph.graphview.labels.LabelSource;
...
public class GraphPanel<V, E> extends Pane {
    ...
    private final Map<com.rnar.umlgraph.graph.Edge<E, V>, EdgeBase> edgeNodes;
    private Map<com.rnar.umlgraph.graph.Edge<E,V>, Tuple<Vertex<V>>> connections;
    ...
    private void initNodes() {
        ...
        List<com.rnar.umlgraph.graph.Edge<E, V>> edgesToPlace = listOfEdges();

        for (Vertex<V> vertex : vertexNodes.keySet()) {

            Iterable<com.rnar.umlgraph.graph.Edge<E, V>> incidentEdges = theGraph.incidentEdges(vertex);

            for (com.rnar.umlgraph.graph.Edge<E, V> edge : incidentEdges) {
    ...
    private EdgeBase createEdge(com.rnar.umlgraph.graph.Edge<E, V> edge, GraphVertexPaneNode<V> graphVertexInbound, GraphVertexPaneNode<V> graphVertexOutbound) {
        /*
...
//            v.attachLabel(label);
    ...
    private void addEdge(EdgeBase e, com.rnar.umlgraph.graph.Edge<E, V> edge) {
        //edges to the back
    ...
    private void insertNodes() {
        ...
        if (!unplottedVertices.isEmpty()) {
            ...
            for (Vertex<V> vertex : unplottedVertices) {
                ...
                double x, y;
                Collection<com.rnar.umlgraph.graph.Edge<E, V>> incidentEdges = theGraph.incidentEdges(vertex);
                if (incidentEdges.isEmpty()) {
                    /* not (yet) connected, put in the middle of the plot */
                    x = mx;
                    y = my;
                } else {
                    com.rnar.umlgraph.graph.Edge<E, V> firstEdge = incidentEdges.iterator().next();
        ...
        Collection<com.rnar.umlgraph.graph.Edge<E, V>> unplottedEdges = unplottedEdges();
        if (!unplottedEdges.isEmpty()) {
            for (com.rnar.umlgraph.graph.Edge<E, V> edge : unplottedEdges) {
    ...
    private void removeNodes() {
        ...
        Collection<com.rnar.umlgraph.graph.Edge<E, V>> removedEdges = removedEdges();
        for (com.rnar.umlgraph.graph.Edge<E, V> e : removedEdges) {
    ...
    * AUTOMATIC LAYOUT
    ...
    private int getTotalEdgesBetween(Vertex<V> v, Vertex<V> u) {
        ...
        int count = 0;
        for (com.rnar.umlgraph.graph.Edge<E, V> edge : theGraph.edges()) {
    ...
    private List<com.rnar.umlgraph.graph.Edge<E, V>> listOfEdges() {
        List<com.rnar.umlgraph.graph.Edge<E, V>> list = new LinkedList<>();
        for (com.rnar.umlgraph.graph.Edge<E, V> edge : theGraph.edges()) {
            list.add(edge);
        }
        return list;
     ...
     */
    private Collection<com.rnar.umlgraph.graph.Edge<E, V>> removedEdges() {
        List<com.rnar.umlgraph.graph.Edge<E, V>> removed = new LinkedList<>();

        Collection<com.rnar.umlgraph.graph.Edge<E, V>> graphEdges = theGraph.edges();
     ...
     * @return collection of edges
     */
    private Collection<com.rnar.umlgraph.graph.Edge<E, V>> unplottedEdges() {
        List<com.rnar.umlgraph.graph.Edge<E, V>> unplotted = new LinkedList<>();

        for (com.rnar.umlgraph.graph.Edge<E, V> e : theGraph.edges()) {
     ...
     * @return stylable element
     */
    public StylableNode getStylableEdge(com.rnar.umlgraph.graph.Edge<E, V> edge) {
    ...
    public StylableNode getStylableEdge(E edgeElement) {
        for (com.rnar.umlgraph.graph.Edge<E, V> e : edgeNodes.keySet()) {
     ...
     * @return stylable element (label)
     */
    public StylableNode getStylableLabel(com.rnar.umlgraph.graph.Edge<E,V> e) {
        ...
        setOnMouseClicked((MouseEvent mouseEvent) -> {
            if (mouseEvent.getButton().equals(MouseButton.PRIMARY)) {
                if (mouseEvent.getClickCount() == 2) {
                    ...
                    Node node = pick(GraphPanel.this, mouseEvent.getSceneX(), mouseEvent.getSceneY());
                    if (node == null) {
                        return;
                    }
