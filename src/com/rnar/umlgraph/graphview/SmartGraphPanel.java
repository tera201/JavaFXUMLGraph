...
import com.rnar.umlgraph.arrows.DiamondArrow;
import com.rnar.umlgraph.arrows.SimpleArrow;
import com.rnar.umlgraph.arrows.TriangleArrow;
...
public class SmartGraphPanel<V, E> extends Pane {
    ...
    private final Map<Vertex<V>, SmartGraphVertexPaneNode<V>> vertexNodes;
    private final Map<Edge<E, V>, SmartGraphEdgeBase> edgeNodes;
    private Map<Edge<E,V>, Tuple<Vertex<V>>> connections;
    private final Map<Tuple<SmartGraphVertexPaneNode>, Integer> placedEdges = new HashMap<>();
    ...
    private void initNodes() {
        ...
        for (Vertex<V> vertex : listOfVertices()) {
            SmartGraphVertexPaneNode<V> vertexAnchor = new SmartGraphVertexPaneNode(vertex, 0, 0,
        ...
        for (Vertex<V> vertex : vertexNodes.keySet()) {
            ...
            for (Edge<E, V> edge : incidentEdges) {
                ...
                SmartGraphVertexPaneNode<V> graphVertexIn = vertexNodes.get(vertex);
                SmartGraphVertexPaneNode<V> graphVertexOppositeOut = vertexNodes.get(oppositeVertex);
        ...
        for (Vertex<V> vertex : vertexNodes.keySet()) {
            SmartGraphVertexPaneNode<V> v = vertexNodes.get(vertex);

            addVertex(v);
        }
    }

    private SmartGraphEdgeBase createEdge(Edge<E, V> edge, SmartGraphVertexPaneNode<V> graphVertexInbound, SmartGraphVertexPaneNode<V> graphVertexOutbound) {
        /*
    ...
    private void addVertex(SmartGraphVertexPaneNode<V> v) {
...
//            v.attachLabel(label);
    ...
    private void insertNodes() {
        ...
        List<SmartGraphVertexPaneNode<V>> newVertices = null;
        ...
        if (!unplottedVertices.isEmpty()) {
            ...
            for (Vertex<V> vertex : unplottedVertices) {
                ...
                if (incidentEdges.isEmpty()) {
                    ...
                    SmartGraphVertexPaneNode<V> existing = vertexNodes.get(opposite);
                ...
                SmartGraphVertexPaneNode newVertex = new SmartGraphVertexPaneNode<>(vertex,
        ...
        if (!unplottedEdges.isEmpty()) {
            for (Edge<E, V> edge : unplottedEdges) {
                ...
                SmartGraphVertexPaneNode<V> graphVertexOut = vertexNodes.get(u);
                SmartGraphVertexPaneNode<V> graphVertexIn = vertexNodes.get(v);
        ...
        if (newVertices != null) {
            for (SmartGraphVertexPaneNode<V> v : newVertices) {
                addVertex(v);
            }
        }
    ...
    private void removeNodes() {
        ...
        for (Edge<E, V> e : removedEdges) {
            ...
            if( getTotalEdgesBetween(vertexTuple.first, vertexTuple.second) == 0 ) {
                SmartGraphVertexPaneNode<V> v0 = vertexNodes.get(vertexTuple.first);
                SmartGraphVertexPaneNode<V> v1 = vertexNodes.get(vertexTuple.second);
        ...
        for (Vertex<V> removedVertex : removedVertices) {
            SmartGraphVertexPaneNode<V> removed = vertexNodes.remove(removedVertex);
    ...
    private void removeVertex(SmartGraphVertexPaneNode v) {
    ...
    private void updateLabels() {
        theGraph.vertices().forEach((v) -> {
            SmartGraphVertexPaneNode<V> vertexNode = vertexNodes.get(v);
            if (vertexNode != null) {
    ...
    private Rectangle2D getPlotBounds() {
        ...
        for (SmartGraphVertexPaneNode<V> v : vertexNodes.values()) {
    ...
    * AUTOMATIC LAYOUT
    ...
    private void computeForces() {
        for (SmartGraphVertexPaneNode<V> v : vertexNodes.values()) {
            for (SmartGraphVertexPaneNode<V> other : vertexNodes.values()) {
                if (v == other) {
                    continue; //NOP
                }
    ...
    private boolean areAdjacent(SmartGraphVertexPaneNode<V> v, SmartGraphVertexPaneNode<V> u) {
    ...
    private Collection<Vertex<V>> removedVertices() {
        ...
        Collection<SmartGraphVertexPaneNode<V>> plotted = vertexNodes.values();

        for (SmartGraphVertexPaneNode<V> v : plotted) {
    ...
    public void setVertexPosition(Vertex<V> v, double x, double y) {
        SmartGraphVertexPaneNode<V> node = vertexNodes.get(v);
        if(node != null) {
    ...
    public SmartStylableNode getStylableLabel(Vertex<V> v) {
        SmartGraphVertexPaneNode<V> vertex = vertexNodes.get(v);
